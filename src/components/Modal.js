...
export default function Modal({ dni, completeButtonRef }) {
  ...
  async function sendEmail(email) {
    let EnviaMailQRInput = {
      ...
      PadOrdenMesa: datos.PadOrdenMesa,
...
const mostrarDatos = () => {
  return (
    ...
    <div className="my-4 py-3 border-2 solid rounded-md  border-gray-400 w-full">
      ...
      <p className="ml-2 text-sm text-gray-400">
        Orden: <span className="text-gray-600">{datos.PadOrdenMesa}</span>
      </p>
      ...
      <img className="w-1/3 block py-2 mx-auto" 
      src={datos.urlPath} 
      alt=""
      />
...
const mostrarErrores = () => {
  ...
  const condiciones = () => {
    ...
    if(loading){
      ...
      return datos ? mostrarDatos() : mostrarErrores()
    }
 }
